...
export const bookInstanceList = asyncHandler(async (req, res, next) => {
  const bookInstances =await  BookInstance.find({}).populate("book", "title").exec();
  console.log(bookInstances);
  res.render("bookInstanceList", { title: "Book instance list", bookInstances });
});

//displays book instance
export const bookInstanceDetails = asyncHandler((req, res, next) => {
  res.send("NOT IMPL.");
});
